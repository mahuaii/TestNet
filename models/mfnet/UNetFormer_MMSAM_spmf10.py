...
from .modules import DSMStructureBranch10, MultiScaleStructurePriorModulatedFusion10


def _resolve_spmf10_indexes(image_encoder: nn.Module) -> list[int]:
    global_indexes = [
        index for index, block in enumerate(image_encoder.blocks) if getattr(block, "window_size", None) == 0
    ]
    if len(global_indexes) != 4:
        raise ValueError(
            "Expected exactly 4 encoder feature taps from global attention blocks for SPMF10, "
...
class UNetFormerSPMF10(UNetFormer):
    def __init__(
        self,
        *args: object,
        ...
        **kwargs: object,
        ...
        self.spmf10_indexes = _resolve_spmf10_indexes(self.image_encoder)
        ...
        self.spmf10 = MultiScaleStructurePriorModulatedFusion10(
            channels=256,
        ...
        if record_intermediate_stats:
            attach_requested_intermediate_stats(
                ...
                {
                    "spmf10": [(self.spmf10, "spmf10")],
                    "structure10": [(self.structure_branch10, "spmf10/structure")],
                },
            )
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, mode: str = "Train") -> torch.Tensor:
        ...
        deepx, deepy, dsm_taps = self._encode_spmf10_features(x, y_sam)
        rgb_feats = (
            self.fpn1x(deepx),
            self.fpn2x(deepx),
            self.fpn3x(deepx),
        ...
        res1, res2, res3, res4 = self.spmf10(rgb_feats, dsm_feats, structure_feats)
        return self.decoder(res1, res2, res3, res4, h, w)

    def _encode_spmf10_features(
        self,
        x: torch.Tensor,
        y: torch.Tensor,
    ...
    @staticmethod
    def _validate_spmf10_taps(dsm_taps: Sequence[torch.Tensor]) -> None:
        if len(dsm_taps) != 4:
            raise ValueError(f"Expected exactly 4 DSM SAM taps for SPMF10, got {len(dsm_taps)}.")


__all__ = ["UNetFormerSPMF10"]
