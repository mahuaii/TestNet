...
from .modules import DSMStructureBranch11, MultiScaleStructurePriorModulatedFusion11


def _resolve_spmf11_indexes(image_encoder: nn.Module) -> list[int]:
    global_indexes = [
        index for index, block in enumerate(image_encoder.blocks) if getattr(block, "window_size", None) == 0
    ]
    if len(global_indexes) != 4:
        raise ValueError(
            "Expected exactly 4 encoder feature taps from global attention blocks for SPMF11, "
...
class UNetFormerSPMF11(UNetFormer):
    def __init__(
        self,
        *args: object,
        ...
        **kwargs: object,
        ...
        self.spmf11_indexes = _resolve_spmf11_indexes(self.image_encoder)
        tap_channels = int(self.image_encoder.embed_dim)
        self.structure_branch11 = DSMStructureBranch11(tap_channels=tap_channels, output_channels=256)
        self.spmf11 = MultiScaleStructurePriorModulatedFusion11(
            channels=256,
        ...
        if record_intermediate_stats:
            attach_requested_intermediate_stats(
                ...
                {
                    "spmf11": [(self.spmf11, "spmf11")],
                    "structure11": [(self.structure_branch11, "spmf11/structure")],
                },
            )
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, mode: str = "Train") -> torch.Tensor:
        ...
        deepx, deepy, dsm_taps = self._encode_spmf11_features(x, y_sam)
        rgb_feats = (
            self.fpn1x(deepx),
            self.fpn2x(deepx),
            self.fpn3x(deepx),
        ...
        structure_feats = self.structure_branch11(dsm, dsm_taps)
        res1, res2, res3, res4 = self.spmf11(rgb_feats, dsm_feats, structure_feats)
        return self.decoder(res1, res2, res3, res4, h, w)

    def _encode_spmf11_features(
        self,
        x: torch.Tensor,
        y: torch.Tensor,
    ...
    @staticmethod
    def _validate_spmf11_taps(dsm_taps: Sequence[torch.Tensor]) -> None:
        if len(dsm_taps) != 4:
            raise ValueError(f"Expected exactly 4 DSM SAM taps for SPMF11, got {len(dsm_taps)}.")


__all__ = ["UNetFormerSPMF11"]
