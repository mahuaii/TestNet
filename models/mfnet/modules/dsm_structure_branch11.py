...
class DSMStructureBranch11(nn.Module):
    """Generate multi-scale structure features from pre-normalized DSM and DSM SAM taps."""
    ...
    def _make_structure_input(self, dsm: torch.Tensor) -> torch.Tensor:
        similarity = self._local_similarity(dsm)
        return torch.cat([dsm, similarity], dim=1)

    def _local_similarity(self, dsm: torch.Tensor) -> torch.Tensor:
