...
class SPMFFusionBlock30(nn.Module):
    """Single-scale SPMF30 block with structure-conditioned evidence routing."""
    ...
    def forward(self, rgb: torch.Tensor, dsm: torch.Tensor, structure: torch.Tensor) -> torch.Tensor:
        ...
        rgb_logits = self.rgb_evidence_head(rgb_modulated)
        dsm_logits = self.dsm_evidence_head(dsm_modulated)
    ...
    def _record_gate_stats(self, *, rgb_weight: torch.Tensor, dsm_weight: torch.Tensor) -> None:
        ...
        if stats is None:
            return
        prefix = str(getattr(self, "intermediate_stats_prefix", "spmf30")).strip("/")
        for name, value in (
...
class MultiScaleSPMFFusion30(nn.Module):
    """Apply independent SPMF30 blocks to four feature scales."""
